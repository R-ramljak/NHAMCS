...
data.design.exc <- svydesign(id = ~id, weight = ~PATWT, data = data, nest = TRUE) # is nest necessary?
...
sample.2c.des <- svydesign(id = ~id, data = sample.2c, nest = TRUE) # is nest necessary?
(result.2c <- svymean(~WAITTIME, sample.2c.des, na.rm = T))
