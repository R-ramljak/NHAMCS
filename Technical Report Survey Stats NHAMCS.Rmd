...
Implement formulas??
implement different sample sizes c to see the decrease of the SE (1000, 5000, 10000)

```{r}
# With the srswr function we can draw a srs, which gives us a vector of the sample cases
sample.2c.vec <- srswr(n = 2000, N = length(data$id))
...
results.2 <- tibble(name.results, estimates = c(result.2a, result.2b, result.2c),
                    SE = c(map_dbl(list(result.2a, result.2b, result.2c), ~sqrt(attr(., which = "var")))))
