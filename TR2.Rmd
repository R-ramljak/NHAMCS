...
stratified.datasets <- list(equal.dataset, pps.dataset, neyman.dataset) %>% 
  map(~getdata(data = data.strata.prep, m = .)) %>% 
  map(~left_join(., strata.sizes, by = "REGION")) %>% 
  pmap(list(., list(.$strata.equal, .$strata.pps, .$strata.neyman)), ~mutate(.x, weights.strata = population.size / .y))

set.seed(87)
# Equal sizes
stratified.equal.sample.vec <- strata(data = data.strata.prep, stratanames = "REGION", 
                                      size = strata.sizes$strata.equal, method = "srswr")
...
stratified.PPS.sample.vec <- strata(data = data.strata.prep, stratanames = "REGION", 
                                    size = strata.sizes$strata.pps, method = "srswr")
